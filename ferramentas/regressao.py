...
A lógica de decisão vive em `pesquisa_precos.core.regressao` (pura, sem I/O) — este script só
...
Uso:
    python ferramentas/regressao.py                              # banco, ou fixture se vazio
    python ferramentas/regressao.py --fonte fixture               # força a fixture sintética
    python ferramentas/regressao.py --t-aceita 0.80 --t-rejeita 0.30
    python ferramentas/regressao.py --reprovar-abaixo-de 0.85     # sai com código 1 se reprovar
"""
...
from pesquisa_precos.core.regressao import Rotulo, avaliar  # noqa: E402
...
def carregar_do_banco(limite: int) -> list[Rotulo] | None:
    ...
    ok, _detalhe = db.esta_disponivel()
    if not ok:
        return None
    with db.sessao() as s:
