...
`DATABASE_URL` do `.env` (mesma fonte da verdade de `db.sessao.url_banco`; nunca uma segunda
...
Uso:
    python ferramentas/backup.py                    # dump em ferramentas/backups/
    python ferramentas/backup.py --destino D:/backups
    python ferramentas/backup.py --verificar caminho/para/arquivo.dump
...
from pesquisa_precos.db.sessao import url_banco  # noqa: E402
...
def main() -> int:
    ...
    print(f"Backup de {url_banco()!r} → {args.destino}")
    arquivo = rodar_pg_dump(url_banco(), args.destino, executavel=args.pg_dump)
