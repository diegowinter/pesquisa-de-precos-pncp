...
`downgrade` dela não teria como desfazer a cifra. `ferramentas/` é exatamente o lugar de script
de apoio pontual (ver CLAUDE.md), e este é pontual por definição.

    uv run python -m ferramentas.semear_provedores --conferir   # não escreve nada
    uv run python -m ferramentas.semear_provedores              # grava
...
def plano() -> tuple[list[dict], list[dict], list[str]]:
    ...
    for capacidade, prefixo in (("pdf", "PDF"), ("pareamento", "PAREAMENTO")):
        ...
        if not base:
            ...
            continue
        provedores.append({
            "nome": f"servico_{capacidade}", "capacidades": [capacidade], "base_url": base,
            "api_key": _env(f"{prefixo}_API_KEY") or None})
        apontamentos.append({"capacidade": capacidade, "provedor": f"servico_{capacidade}"})
...
def executar(conferir: bool = False) -> int:
    ...
    if not seg.configurada():
        print(f"\n✖ {seg.VAR_CHAVE} não está no ambiente — sem ela não dá para cifrar as "
              ...
              f"    uv run python -c \"from pesquisa_precos.db import segredo; "
