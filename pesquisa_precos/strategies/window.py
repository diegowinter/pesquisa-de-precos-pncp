...
validação — só o transporte (agora devolve os campos para `steps.e5_extract` gravar, em vez
...
def extrair_item(curador, texto_doc: str, item: dict, *, janela_max: int = MAX_JANELA,
    ...
    `strategies.base.validar_extracao` espera (`descricao_completa`/`preco_unitario`/
