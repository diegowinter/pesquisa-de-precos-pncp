...
def _chave_atual() -> bytes:
    ...
    if not value:
        raise ChaveMestraAusente(
            ...
            f"a key de API dos provedores (ADR-022).")
...
def configurada() -> bool:
    """Para a tela de diagnóstico dizer 'key-mestra ausente' sem derrubar a página."""
    return bool(os.getenv(VAR_CHAVE, ""))


def cifrar(segredo: str, *, context: str) -> bytes:
    """`context` é o AAD — na prática, `provider.name`. Ver docstring do módulo."""
    material = _chave_atual()
    kid = _key_id(material).encode("ascii").ljust(_TAM_KEY_ID, b"\x00")
    nonce = os.urandom(_TAM_NONCE)
    cripto = AESGCM(material).encrypt(nonce, segredo.encode("utf-8"), context.encode("utf-8"))
...
def decifrar(blob: bytes, *, context: str) -> str:
    ...
    raise SegredoInvalido(
        f"não foi possível decifrar o segredo de {context!r} com a(s) key-mestra(s) "
...
def key_id_do_blob(blob: bytes) -> str | None:
    """Qual key-mestra cifrou este blob, sem tentar decifrar — é o que permite listar o que
