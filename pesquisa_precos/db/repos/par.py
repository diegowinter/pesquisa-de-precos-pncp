...
`final_decision` é derivada: `confirmado` = (`decisao='aceito'`) OU (`veredito='sim'`).
...
COLUNAS_ROTULO = ("par_key", "texto_catalogo", "texto_item", "score_rerank",
                  "final_decision", "source", "model", "run_id")
...
def recomputar_decisao_final(sessao: Session) -> int:
    ...
    return sessao.execute(text("""
        ...
        WHERE final_decision IS DISTINCT FROM (CASE
            WHEN decisao = 'aceito' OR veredito = 'sim' THEN 'confirmado'
            WHEN veredito IN ('nao', 'indeterminado') THEN 'rejeitado'
            WHEN decisao = 'rejeitado' AND veredito IS NULL THEN 'rejeitado'
            ELSE 'pendente'
...
def gravar_embeddings(conn: psycopg.Connection, provider: str, model: str,
    ...
    """
    linhas = []
    for texto_hash, vector in itens:
        v = np.asarray(vector, dtype="<f2")
        linhas.append((texto_hash, provider, model, int(v.size), v.tobytes()))
    return copy.copiar(
        conn, "embedding_cache",
        ("texto_hash", "provider", "model", "dimension", "vector"), linhas,
        conflito=("texto_hash", "provider", "model", "dimension"),
    )
...
def amostra_rotulos(sessao: Session, limite: int = 200) -> list[dict]:
    """Amostra de `label` para a suite de regressão (Fase 9) — os mais recentes primeiro
    (`created_at DESC`), até `limite` linhas. `label` é append-only e cresce entre execuções
    (docs/02_SCHEMA.md §7); os mais recentes refletem melhor o estado atual do reranker."""
    linhas = sessao.execute(
        text("SELECT par_key, score_rerank, final_decision FROM label "
             "ORDER BY created_at DESC LIMIT :n"), {"n": limite},
    ).mappings().all()
    return [dict(r) for r in linhas]
...
def contar(sessao: Session) -> dict[str, int]:
    q = {
        ...
        "par_confirmado": "SELECT count(*) FROM par WHERE final_decision = 'confirmado'",
        "label": "SELECT count(*) FROM label",
