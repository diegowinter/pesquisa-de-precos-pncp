...
canal Resend/e-mail somado ao Telegram já existente). Credenciais (API key, bot token) não
moram aqui — só em `.env` (ADR-006); esta tabela guarda apenas QUEM recebe.
"""
...
def listar(sessao: Session, *, apenas_ativos: bool = False) -> list[dict[str, Any]]:
    ...
    linhas = sessao.execute(text(
        f"SELECT id, nome, email, telegram_chat_id, ativo, criado_em "
...
def obter(sessao: Session, destinatario_id: int) -> dict[str, Any] | None:
    linha = sessao.execute(text(
        "SELECT id, nome, email, telegram_chat_id, ativo, criado_em "
        "FROM notificacao_destinatario WHERE id = :id"),
        {"id": destinatario_id}).mappings().first()
    return dict(linha) if linha else None


def criar(sessao: Session, nome: str | None, email: str | None,
         telegram_chat_id: str | None) -> int:
    return sessao.execute(
        text("INSERT INTO notificacao_destinatario (nome, email, telegram_chat_id) "
             "VALUES (:n, :e, :t) RETURNING id"),
        {"n": nome or None, "e": email or None, "t": telegram_chat_id or None},
    ).scalar_one()


def editar(sessao: Session, destinatario_id: int, nome: str | None, email: str | None,
          telegram_chat_id: str | None) -> int:
    """Devolve o número de linhas afetadas (0 = id inexistente)."""
    return sessao.execute(
        text("UPDATE notificacao_destinatario "
             "SET nome = :n, email = :e, telegram_chat_id = :t "
             "WHERE id = :id"),
        {"id": destinatario_id, "n": nome or None, "e": email or None,
         "t": telegram_chat_id or None},
    ).rowcount
