...
# si — a etapa só enxerga `RunContext` (docs/03_ETAPAS.md §1). `run.teto_custo_usd` é
...
def erros_do_run(sessao: Session, run_id: int, *, etapa: str | None = None,
    ...
    """`erro_item` do run — a Fase 4 expõe isto em `GET /api/runs/{id}/errors` para o gate/tela
...
def atualizar_status_provedor(sessao: Session, provedor: str, saudavel: bool,
    ...
    """Resultado de UMA sondagem (`providers.health`) — `provedor_status` é sempre a última
