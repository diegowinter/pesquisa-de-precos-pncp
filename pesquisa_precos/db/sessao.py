...
`conexao_bruta()` devolve a conexão psycopg pura porque a migração usa `COPY`, que é a única
...
def engine() -> Engine:
    ...
    if _engine is None:
        _engine = create_engine(url_banco(), pool_pre_ping=True, future=True)
...
def esta_disponivel() -> tuple[bool, str]:
