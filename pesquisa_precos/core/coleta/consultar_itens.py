...
Uso:
    python consultar_itens.py --cnpj 16695025000197 --ano 2025 --seq 679
"""
...
def main():
    parser = argparse.ArgumentParser(description="Consultar itens de uma compra - PNCP")
    parser.add_argument("--cnpj", required=True)
    parser.add_argument("--ano", required=True)
    parser.add_argument("--seq", required=True)
    args = parser.parse_args()

    itens = fetch_itens(args.cnpj, args.ano, args.seq)
    homologados = filtrar_homologados(itens)
    print(f"Total itens: {len(itens)} — Homologados: {len(homologados)}")
    print(json.dumps(homologados[:5], indent=2, ensure_ascii=False))
