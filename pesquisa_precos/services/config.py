...
editar sempre nasce como versão nova. `run.config_version_id` é quem resolve "qual versão gerou
...
from pesquisa_precos.core.regression import Label, avaliar
...
class ConfigVersaoInexistente(RuntimeError):
    """`config_version_id` não existe — 404 na API."""
...
def obter_config_versao(config_version_id: int) -> dict[str, Any] | None:
    with db.session() as sessao:
        return repo.config_versao_por_id(sessao, config_version_id)


def criar_config_versao(label: str, valores: dict[str, Any], *,
                        created_by: str | None = None, notes: str | None = None) -> int:
    with db.session() as sessao:
        config_version_id = repo.criar_config_versao(sessao, label, created_by=created_by,
                                                     notes=notes)
        repo.gravar_config(sessao, config_version_id, valores)
...
def diff_config_versoes(id_a: int, id_b: int) -> dict[str, Any]:
    with db.session() as sessao:
        if repo.config_versao_por_id(sessao, id_a) is None:
            raise ConfigVersaoInexistente(f"config_version {id_a} não existe")
        if repo.config_versao_por_id(sessao, id_b) is None:
            raise ConfigVersaoInexistente(f"config_version {id_b} não existe")
...
def schema_parametros() -> dict[str, Any]:
    ...
    for definicao in registry.ordem():
        campos = {}
        for name, campo in definicao.params_model.model_fields.items():
            campos[name] = {
                "tipo": str(campo.annotation),
                "default": campo.get_default(call_default_factory=True),
                "descricao": campo.description or "",
            }
        saida[definicao.key] = {"titulo": definicao.titulo, "campos": campos}
    return saida
