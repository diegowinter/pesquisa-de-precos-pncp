...
A lease (`expira_em`) é quem resolve o caso em que nem a conexão caiu de forma limpa: se o
...
def tentar_adquirir(sessao: Session, run_etapa_id: int, pid: int, *,
    ...
    linha = sessao.execute(
        text("INSERT INTO execucao_lock (id, run_etapa_id, pid, adquirido_em, expira_em) "
             "VALUES (1, :re, :pid, now(), now() + make_interval(secs => :t)) "
             "ON CONFLICT (id) DO UPDATE SET "
             "  run_etapa_id = EXCLUDED.run_etapa_id, pid = EXCLUDED.pid, "
             "  adquirido_em = EXCLUDED.adquirido_em, expira_em = EXCLUDED.expira_em "
             "WHERE execucao_lock.run_etapa_id IS NULL "
             "   OR execucao_lock.expira_em IS NULL "
             "   OR execucao_lock.expira_em < now() "
...
def renovar(sessao: Session, run_etapa_id: int, *, timeout_s: int = LEASE_PADRAO_S) -> None:
    ...
    sessao.execute(
        text("UPDATE execucao_lock SET expira_em = now() + make_interval(secs => :t) "
...
def liberar(sessao: Session, run_etapa_id: int) -> None:
    ...
    sessao.execute(
        text("UPDATE execucao_lock SET run_etapa_id = NULL, pid = NULL, "
             "                          adquirido_em = NULL, expira_em = NULL "
...
def quem_detem(sessao: Session) -> dict | None:
    ...
    linha = sessao.execute(
        text("SELECT run_etapa_id, pid, adquirido_em, expira_em FROM execucao_lock "
