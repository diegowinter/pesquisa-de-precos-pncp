"""
Ponto de entrada do subprocesso de uma etapa (ADR-002) — o que `runner.executor` sobe.

Uso: `python -m pesquisa_precos.runner.processo <run_etapa_id>`

O `run_etapa` já existe e já está `executando` quando este processo começa (é
`executor.iniciar_subprocesso` quem faz isso, ANTES de subir o subprocesso — assim, se o
...
def executar(run_etapa_id: int) -> int:
    sessao_execucao = db.criar_sessao()
...
def main() -> None:
    if len(sys.argv) != 2:
        print("uso: python -m pesquisa_precos.runner.processo <run_etapa_id>", file=sys.stderr)
        raise SystemExit(2)
