"""
m02 — Prompts: `core/prompts.py` → `prompt` + `prompt_versao` (v1 ativa).

Na Fase 2 o objetivo é modesto e específico: dar a `texto_classificacao.prompt_versao_id` e a
`llm_chamada.prompt_versao_id` uma linha real a que apontar. Migrar a EDIÇÃO de prompt para a
...
valor (`"{descricao}"`), o que devolve o texto real com placeholders no lugar certo. É o mais
...
DEFINICOES = (
    ("classificar_item",
     ...
     "chat",
     lambda: prompts.montar_prompt_classificar_item("{descricao}", "{unidade}")),
    ("termos_item",
     "Etapa 1 — gera termos de busca genéricos a partir de um item do catálogo",
     "chat",
     lambda: prompts.montar_prompt_termos_item("{nome}", "{descricao}", "{tipo}",
                                               "{nome_grupo}")),
    ("extrair_item_pdf",
    ...
    ("comparar_item",
     "Comparação item PNCP × item de catálogo com o objeto da compra em contexto",
     "chat",
     lambda: prompts.montar_prompt_comparar_item(
         "{descricao_pncp}", "{objeto_compra}", "{nome_catalogo}", "{descricao_catalogo}")),
    ("busca_termos",
     "Geração de termos de busca a partir de nome/descrição de catálogo",
     "chat",
     lambda: prompts.montar_prompt_busca("{nome}", "{descricao}", "{categoria}")),
    ("extrair_tabela_pdf",
    ...
    with db.session() as s:
        for nome, descricao, capacidade, montar in DEFINICOES:
            try:
                ...
                rel.aviso(f"{nome}: não renderizou ({type(exc).__name__}: {exc})")
                rel.mais("falhou")
                continue
            repo.upsert_prompt(s, nome, descricao, capacidade, template, versao=1, ativa=True)
            rel.mais("prompt_versao v1 ativa")
    return rel
