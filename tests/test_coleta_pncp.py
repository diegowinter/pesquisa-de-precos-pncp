...
class TestMontarItemKey:
    def test_combina_controle_e_numero(self):
        assert collect_pncp.montar_item_key("123-1-000456/2026", 3) == "123-1-000456/2026::3"
...
class TestBaseResultado:
    def test_contrato_usa_numero_sequencial_direto(self):
        ...
        base = collect_pncp._base_resultado(r, "contrato")
    ...
    def test_ata_usa_numero_sequencial_compra_e_guarda_seq_proprio(self):
        ...
        base = collect_pncp._base_resultado(r, "ata")
        assert base["numero_sequencial"] == "456"
    ...
    def test_uf_com_fallback_para_campo_uf(self):
        ...
        assert collect_pncp._base_resultado(r, "contrato")["uf"] == "RJ"
    ...
    def test_data_prioriza_publicacao_pncp(self):
        ...
        assert collect_pncp._base_resultado(r, "contrato")["data"] == "2026-01-05"
...
class TestColetarDeBaseSemIdentificacao:
    def test_sem_cnpj_ano_ou_sequencial_e_sem_identificacao(self):
        ...
        linhas, status = collect_pncp._coletar_de_base(base, "contrato", "termo x")
...
class TestPrecoHomologadoVsEstimado:
    ...
    @patch("pesquisa_precos.core.collection.fetch_files.listar_arquivos")
    @patch("pesquisa_precos.core.collection.fetch_files.selecionar_do_tipo")
    @patch("pesquisa_precos.core.collection.fetch_items.resolver_sequencial_compra_contrato")
    @patch("pesquisa_precos.core.collection.fetch_items.fetch_itens")
    @patch("pesquisa_precos.core.collection.fetch_items.filtrar_homologados")
    @patch("pesquisa_precos.core.collection.fetch_items.fetch_resultado_vencedor")
    def test_usa_homologado_quando_ha_resultado(
        ...
        linhas, status = collect_pncp._coletar_de_base(self._base_valida(), "contrato", "termo x")
        assert status == "ok"
...
class TestFetchResultadoVencedor:
    ...
    def test_lista_vazia_devolve_none(self, mock_get):
        mock_get.return_value = []
        assert fetch_items.fetch_resultado_vencedor("123", 2026, "9", 1) is None
