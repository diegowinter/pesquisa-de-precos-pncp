...
from pesquisa_precos.runner import executor, fingerprint, lock
...
def test_leases_expiradas_devolve_run_etapa_travada_a_fila(run_id):
    with db.sessao() as sessao:
        ...
        sessao.execute(text(
            ...
            {"id": re_id})
    presas = executor.recuperar_travados(timeout_s=60)
...
def test_contexto_banco_gastar_respeita_teto(run_id):
    with db.sessao() as sessao:
        re_id = repo.obter_ou_criar_run_etapa(sessao, run_id, "3")
    with db.sessao() as db_etapa, db.sessao() as sessao_execucao:
        ctx = ContextoBanco(
            db_etapa, sessao_execucao=sessao_execucao, run_id=run_id, run_etapa_id=re_id,
            etapa="3", acao="atualizar", modo="assistido", config={}, teto_custo_usd=1.0)
        ctx.gastar(0.6)
...
def test_contexto_banco_sem_teto_nao_levanta(run_id):
    ...
    with db.sessao() as db_etapa, db.sessao() as sessao_execucao:
        ctx = ContextoBanco(
            db_etapa, sessao_execucao=sessao_execucao, run_id=run_id, run_etapa_id=re_id,
            etapa="3", acao="atualizar", modo="assistido", config={}, teto_custo_usd=None)
...
# ── executor.preparar: resolução de params em camadas (docs/03_ETAPAS.md §3) ─────────
...
def test_preparar_aplica_camada_de_config_sobre_o_default(run_id):
    ...
    re_id = executor.preparar(run_id, "0a")
    with db.sessao() as sessao:
        linha = repo.run_etapa_por_id(sessao, re_id)
...
def test_preparar_override_vence_config(run_id):
    with db.sessao() as sessao:
        repo.gravar_config(sessao, repo.run_por_id(sessao, run_id)["config_versao_id"],
                           {"so_grupos_seguranca": False})
    re_id = executor.preparar(run_id, "0a", params_override={"so_grupos_seguranca": True})
    with db.sessao() as sessao:
        linha = repo.run_etapa_por_id(sessao, re_id)
