...
def test_settings_so_carrega_o_bootstrap():
    ...
    ganhar um leitor de `.env` de novo, o valor efetivo deixa de ser o que `config_versao`
    ...
    origem = Path(inspect.getfile(settings)).read_text(encoding="utf-8")
    assert "os.getenv" not in origem, "settings.py voltou a ler variável de ambiente"
...
def test_camada_config_versao_sobrepoe_o_default():
    """`runner.launcher.preparar` resolve `Params` em três camadas (default ← config_versao ←
