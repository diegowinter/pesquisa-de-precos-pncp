...
> **Mudança de rumo (2026-07-09): curadoria por LLM removida.** A seleção do catálogo deixou de ser feita pela Etapa 0b (curadoria em 2 rounds de LLM) e passou a ser um **filtro por allow-list** aplicado dentro da 0a: materiais por `codigoPdm`, serviços por `codigoServico` (constantes em `scripts/catalogo_local.py`). A saída curada `data/0b_catalogo_curado.csv` foi substituída por `data/0a_catalogo_filtrado.csv`, e todas as etapas que a consumiam foram reapontadas. A Etapa 0b e o script `0b_curar_catalogo.py` foram aposentados.
...
- Scripts de etapa ficam na **raiz** do projeto, nomeados `{N}{letra?}_{acao}.py` (ex.: `0a_obter_catalogo.py`, `6a_gerar_pares.py`).
- **Toda saída de dados leva o prefixo do script que a produziu**: `data/{N}{letra?}_{descricao}.{csv|parquet|xlsx}`. Ex.: `2_coletar_pncp.py` → `data/2_itens_coletados.csv`. Isso é regra dura: olhar o nome do arquivo deve dizer imediatamente quem o gerou e quem o consome (a etapa seguinte).
- Checkpoints/caches internos (não são saída de etapa) ficam em `data/checkpoints/{N}_...`.
- Logs de erro ficam em `data/erros/{N}_erros.csv`, sempre via `scripts/erros_log.py`.
- Módulos auxiliares (bibliotecas, sem `__main__` de pipeline) ficam em `scripts/`. **Nenhum script numerado pode ser importado por outro script** — se dois scripts precisam da mesma função, ela vai para `scripts/`.
...
2. Escreve resultados incrementalmente com `scripts/io_seguro.EscritorSeguro` (append + fsync por linha, crash-safe).
...
- Toda chamada de LLM (paga ou local via LM Studio) passa por `scripts/llm_curador.py`. Nenhum script chama API de LLM diretamente.
- Embeddings e reranking **não** passam pelo `llm_curador`: usam `scripts/embedder_local.py` e `scripts/reranker_local.py` (sentence-transformers em processo).
- OCR usa `scripts/ocr_pdf.py` (chama o servidor de OCR local via HTTP OpenAI-compatible).
...
- **Filtro (allow-list)**: constantes em `scripts/catalogo_local.py` — `PDMS_MATERIAIS` (filtra materiais por `codigoPdm`) e `CODIGOS_SERVICOS` (filtra serviços por `codigoServico`); função `filtrar_curado(tipo, df)`. Editar essas constantes é como se ajusta o escopo do catálogo (antes era curadoria por LLM).
- **LLM**: não.
...
2. **Categoria nunca vazia**: LLM → se vazia, maioria da categoria dentro do mesmo `codigo_pdm` → mapa grosseiro `nome_grupo → categoria` (`scripts/variacoes.py`) → `outros`.
3. **Variações de grafia** (`scripts/variacoes.py`, definidas à mão): se qualquer forma de um grupo aparecer nos termos do item, todas entram (pick-up/pickup/picape; rádio/transceptor/ht; colete balístico/à prova de bala…). Cada termo também é duplicado na forma sem acento.
...
### Etapa 2 — `2_coletar_pncp.py` ✅

Sucessor do par `1_obter_itens.py` + `1_obter_itens_catalogo.py`. **Reaproveita** as libs `scripts/buscar_pncp.py`, `scripts/consultar_arquivos.py`, `scripts/consultar_itens.py` — a lógica de busca/paginação/filtro homologado do `1_obter_itens.py` atual migra para uma lib `scripts/coleta_pncp.py` (funções puras, sem `__main__`), eliminando o hack de `importlib` para módulo que começa com dígito.
...
- **Como atualizar `conceitos_origem` de linha já escrita mantendo append-only**: não reescrever o CSV principal; gravar os acréscimos em `data/checkpoints/2_conceitos_extra.csv` (`item_key, conceito`) e fazer o merge (groupby + união de conceitos) em memória na leitura — fornecer a função `carregar_itens_coletados()` em `scripts/coleta_pncp.py` que já devolve o DataFrame consolidado. Todas as etapas seguintes leem por essa função, nunca o CSV cru.
- **Chave de resumo**: `(termo, tipo_doc, pagina)` em `data/checkpoints/2_progresso.csv`.
- **LLM**: não.

### Etapa 3 — `3_classificar_itens.py` ✅
...
- **Paralelismo**: mesmo pool de `scripts/paralelo.py`; com LM Studio local, concorrência baixa (2-4) para não estourar a fila do servidor.

### Etapa 4 — `4_cortar_minimo.py` ✅
...
**5.1 Parse + roteamento OCR** (lib `scripts/ocr_pdf.py`):
...
2. **Score léxico**: BM25 (via `rank_bm25`, lib `scripts/indice_lexical.py`) — corpus = descrições finais dos itens PNCP (tokenização: lowercase, sem acento, split alfanumérico); query = nome+descrição do item de catálogo. Guardar score normalizado por categoria (min-max dentro da categoria).
3. **Score semântico**: cosseno entre embeddings bge-m3 (lib `scripts/embedder_local.py`). Embeddings computados **uma vez por texto único** e cacheados: `data/checkpoints/6a_emb_catalogo.parquet` e `data/checkpoints/6a_emb_itens.parquet` (chave = hash do texto). Não precisa de banco de vetores: os pares já estão definidos, é cosseno direto par a par (numpy, vetorizado por categoria).
...
### Etapa 6b — `6b_rerankear_pares.py` ✅

Cross-encoder local decide a maioria dos pares, custo zero de token.

1. Para cada par sobrevivente de 6a: score do reranker (`scripts/reranker_local.py`, `CrossEncoder(RERANKER_MODEL)`, fp16, batch 16-32) sobre o par de textos `(nome + descricao do catálogo, descricao_final do item)`. Truncar cada lado a ~512 tokens.
...
### Etapa 6c — `6c_validar_pares_llm.py` ✅
