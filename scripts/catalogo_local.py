...
(ver obter_catalogo.py). Este módulo:
...
Uso (teste isolado):
    python scripts/catalogo_local.py --descricao "pistola"
    python scripts/catalogo_local.py --descricao "colete balistico" --tipo material
"""
...
PARQUET_MATERIAIS = _DATA_DIR / "0a_catalogo_materiais.parquet"
PARQUET_SERVICOS = _DATA_DIR / "0a_catalogo_servicos.parquet"
...
def carregar_catalogo(tipo: str) -> pd.DataFrame:
    ...
    if not caminho.exists():
        raise FileNotFoundError(
            ...
            f"Rode 'python obter_catalogo.py' primeiro."
        )
